...
class Teleop(Node, ABC):
    def __init__(self):
        ...
        if self.get_parameter("twist_stamped_enabled").value:
            ...
            self.publisher_ = self.create_publisher(
                Twist, "/AMR2/rover_twist", qos_profile_system_default
            )
    ...
    def _publish(self):
        # print(self.angular)
        if int(self.linear) == 0:
            self.new_linear = 0.0
        elif int(self.linear) == 1:
            self.new_linear = 0.3
        elif int(self.linear) == 2:
            self.new_linear = 0.5
        elif int(self.linear) == 3:
            self.new_linear = 1.0
        elif int(self.linear) == 4:
            self.new_linear = 1.5

        # if int(self.angular) == 1:
        #     self.new_angle = 0.1
        # elif int(self.angular) == 2:
        #     self.new_angle = 0.2
        if int(self.angular) == 1:
            self.new_angle = 0.4
        elif int(self.angular) == 2:
            self.new_angle = 0.8
        elif int(self.angular) == 3:
            self.new_angle = 1.2
        # elif int(self.angular) == 5:
        #     self.new_angle = 1.2
        # elif int(self.angular) == -1:
        #     self.new_angle = -0.1
        # elif int(self.angular) == -2:
        #     self.new_angle = -0.2
        elif int(self.angular) == -1:
        ...
        # elif int(self.angular) == -5:
        #     self.new_angle = -1.2
        elif int(self.angular) == 0:
            self.new_angle = -0.0
            
        twist = self._make_twist(self.new_linear, self.new_angle)
        self._update_screen()
        self.publisher_.publish(twist)

    def _update_screen(self):
        sys.stdout.write(f"Linear: {self.new_linear:.2f}, Angular: {self.new_angle:.2f}\r")
